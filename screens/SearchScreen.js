...
export default class Searchscreen extends React.Component {
   ...
   componentDidMount = async()=>{
     const query = await db.collection("transaction").limit(6).get();
   ...
   fetchMoreTransactions = async()=>{
    ...
    if(enterText[0].toUpperCase()==='B'){
      const query = await db.collection("transaction").where("bookId","==",text).startAfter(this.state.lastVisibleTransaction).limit(6).get();
      ...
      const query=await db.collection("transaction").where("studentId", "==", text).startAfter(this.state.lastVisibleTransaction).limit(6).get();
      query.docs.map((doc)=>{
        this.setState({
          allTransactions:[...this.state.allTransactions, doc.data()],
          lastVisibleTransaction:doc
